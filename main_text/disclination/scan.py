...
print(ham.shape,X.shape,Y.shape)
    
print(sp.linalg.ishermitian(ham))
...
vs_locs = np.zeros((scan_res,scan_res,))

wif_index = len(X_U)

mus = np.zeros((scan_res,scan_res))
...
extent = list(scan_range)*2


fname = f"./h5_files/disc_{phase}_scan_res={scan_res}.h5"

hdf.save_numpy_to_hdf5(fname,mus,"/mus")
hdf.save_numpy_to_hdf5(fname,np.array(extent),"/extent")

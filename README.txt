...
This code has been tested on Ubuntu 22.04, and is written to be compatible with UNIX based operating systems (e.g., slashes in file paths are /, not \ as one has in Windows). For Windows users, we note the existence of Ubuntu on WSL (https://ubuntu.com/wsl), which may allow one to run the code directly using the provided .sh scripts without any OS-related refactoring, such as the backslash convention in file paths.
...
A test run of run_everything.sh produces the terminal output :
...
This is accessed via the h5py Python package, but HDF5 often must be separately installed on the machine (see https://www.hdfgroup.org/download-hdf5/). 
